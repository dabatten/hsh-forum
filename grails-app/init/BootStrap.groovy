import hsh_forum.Comment
import hsh_forum.DiscussionThread
import hsh_forum.Document
import hsh_forum.SecRole
import hsh_forum.SecUser
import hsh_forum.SecUserSecRole
import hsh_forum.Section
import hsh_forum.Topic
class BootStrap {
    ...
    def init = { servletContext ->
        ...
        def document = new Document(filename:'testFile.txt', fullPath:'/Users/davisbatten/Documents/hsh/attachments/testFile.txt').save()
            ...
            ('A'..'C').each { sectionLetter ->
                ...
                (1..4).each { topicNumber ->
                    ...
                    (1..numberOfThreads).each { threadNo ->
                        def opener = users[random.nextInt(50)]
                        ...
                        new Comment(thread:thread, commentBy:opener, body:generateRandomComment(), attachments:[document]).save()
                        // create 10-35 replies per thread
                        def numberOfReplies = random.nextInt(26)+10
                        numberOfReplies.times {
                            def commentBy = users[random.nextInt(50)]
